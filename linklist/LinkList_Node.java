...
(LinkList_Node<Integer> head,int position){
	...
	while(temp!=null) {
		if(	i==position+1) {
            ...
            if(temp.next==null){//if some one enter non exist index
                return head;
            }
		...
		//temp.next=temp.next.next;//index 1 reference to get
...
takeInput(){//vehicle v=head; caught reference and utilize it.
	...
	LinkList_Node<Integer> head =null ; // new LinkList_Node<Integer>();//=null;
